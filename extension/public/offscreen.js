// Overlai offscreen recorder (plain JS — copied verbatim from public/, no bundling).
//
...
async function startRecording() {
  ...
  try {
    ...
    if (perm.state !== 'granted') {
      recording = false
      send({ type: 'OVERLAI_AUDIO_ERROR', error: 'permission: microphone not granted' })
      return
    }
  } catch {
...
function finalize(mime) {
  recording = false
  if (!chunks.length) {
    send({ type: 'OVERLAI_AUDIO_ERROR', error: 'empty recording' })
    return
  }
  const blob = new Blob(chunks, { type: mime })
  const reader = new FileReader()
  reader.onloadend = () => {
    send({ type: 'OVERLAI_AUDIO', audio: reader.result, mimeType: mime })
  }
  reader.onerror = () => send({ type: 'OVERLAI_AUDIO_ERROR', error: 'failed to encode audio' })
