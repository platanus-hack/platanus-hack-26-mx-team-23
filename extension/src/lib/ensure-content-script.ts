// Ensures the Overlai content script is present in a tab before we message it.
//
...
// have it — sending OVERLAI_TEXT there silently no-ops ("Receiving end does not
...
export async function ensureContentScript(tabId: number): Promise<void> {
  ...
  try {
    ...
    console.warn('[Overlai] could not inject content script into tab', tabId, err)
    return
  }
...
async function ping(tabId: number): Promise<boolean> {
  try {
    const res = await chrome.tabs.sendMessage(tabId, { type: 'OVERLAI_PING' })
    return res?.ok === true
  } catch {
    return false
  }
